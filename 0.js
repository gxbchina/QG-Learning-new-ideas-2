...
if (storage.get(engine_version, true)) {
  ...
  let gengxin_rows = "脚本有风险，仅供学习交流;更新内容：;1.更换稳定代理网站;2.增加优化防检测模块文章、视听、本地随机任务;BUG可提交给机器人;3.脚本测试环境：强国V2.48.0-2.53（点击取消不再提示）".split(";");
...
function 滑动滑块() {
  ...
  function detect(str, img) {

    //var img = images.read("/sdcard/0.jpg");
    // var str = res[0].text;
    // var str = google_ocr_api(img)
    // log(str)
    //var str = "拖动滑块直到出现：2个鞋：后松开"
    var numArr = ["零", "一", "二", "三", "四", "五", "六", "七"]
    var labelArr = ["树", "宇航员", "汉堡包", "伞", "花盆", "鸭子", "蝴蝶", "蘑菇", "足球", "包", "吉他", "企鹅", "铲土机", "耳机", "枕头", "鱼", "树袋熊", "气球", "马", "鸟", "帽子", "椅子", "房子", "篮子", "鞋", "凤梨", "闹钟", "玫瑰花", "乌龟", "轮胎", "苹果", "帐篷", "挂锁", "瓶子", "冰激凌", "南瓜", "松鼠", "兔子", "太阳花", "裙子", "大象", "杯子", "草莓", "车", "蛋糕", "皇冠", "熊猫", "瓢虫"];

    var num = -1;
    for (var i = 0; i < numArr.length; i++) {
      var arri = numArr[i];
      if (str.indexOf(arri) >= 0 || str.indexOf(i) >= 0) {
        num = i;
        fInfo(num)
      }
    }
    ...
    for (var i = 0; i < labelArr.length; i++) {
      var arri = labelArr[i];
      if (str.indexOf(arri) >= 0) {
        label = i //labelArr[i];
        fInfo(label)
      }
    }
    if (num == -1 || label == -1) {
      fInfo("识别失败 " + num + " " + label);
      return;
    }
    var bbox = yolo.detect(img.mat, 0.1, 0);
    fInfo("识别到个数 " + bbox.length);
    bbox.sort((a, b) => {
      return a[0] - b[0]
    });
    var num2 = 1;
  ...
  function drawLine(x1, y1) {
    ...
    threads.start(function () {
      ...
      w.board.on("draw", (canvas) => {
        canvas.drawLine(x1, y1, x1, y1 + 400, paint);
      });
      w.setSize(-1, -1);
      sleep(1000)
    })
  ...
  function touch() {
    //按下时的x与y，不松手
    return {
      autojsGestures: function (strokes) {
      ...
      touchDown: function (x, y) {
        ...
        //
        this.currentStroke = new GestureDescription.StrokeDescription(this.path, 0, 1, true);
        this.autojsGestures([this.currentStroke]);
      },
      touchMove: function (x, y, duraction) {
        var duraction = duraction || 200;
        //
        this.path = new Path();
        ...
        this.path.lineTo(x, y);
        //
        this.currentStroke = this.currentStroke.continueStroke(this.path, 0, duraction, true);
        this.autojsGestures([this.currentStroke]);
      },
      touchUp: function () {
        var duraction = duraction || 1;
        //
        this.path = new Path();
        this.path.moveTo(this.x, this.y);
        this.path.lineTo(this.x, this.y);
        //
        this.currentStroke = this.currentStroke.continueStroke(this.path, 0, duraction, false);
        this.autojsGestures([this.currentStroke]);
      },
    }

  }
  //textContains("请按照说明拖动滑块").findOne(1000);
  
  var n = className("android.view.View").text("").findOne(1000);
  if (n) {
    fInfo("开始滑块");
    var sx = 120;
    var sy = 1500;
    ...
    t.touchDown(sx, sy);
    for (var i = 0; i < d; i++) {
      t.touchMove(sx + dx * i, ey + dy * i, d + 1 - i);
    }
    // bounds("(60,690,1020,1332)")
    console.hide();
    sleep(200);
    var img = captureScreen();
    fInfo("截屏完成");
    //bounds("(444,690,687,759)")
    //bounds("(444,720,687,789)")
    // img = images.clip(img, 60,720 ,1020 -60,1362 -720)
    // img2 = images.clip(img, 444+40,720,687-444-40,789-720);
    images.save(img, "/sdcard/学习新思想/1.jpg")
    //img2 = images.clip(img, 430 + 50, 700, 687 - 444 - 40, 789 - 720);
    // img2图形验证提示
    img2 = images.clip(img, 440 + 30, 700, 687 - 455 - 40, 789 - 720);
    // img3图形验证识别
    img3 = images.clip(img, 60, 690, 1020 - 60, 1332 - 714);
    images.save(img2, "/sdcard/学习新思想/2.jpg")
    fInfo("开始识别");
    var str = google_ocr_api(img2)
    fInfo('读取提示：'+str)
    var res_data = detect('拖动滑块：'+str, img3);
    fInfo(res_data+"")
    if (res_data) {
      drawLine(60 + res_data, 714)
      var sx = ex;
      var ex = 60 + res_data + random(-4, -3);
      var dx = (ex - sx) / d;

      for (var i = 0; i < d; i++) {
        t.touchMove(sx + dx * i, ey + dy * (d - i), d - i);
      }
    }
    t.touchUp();
