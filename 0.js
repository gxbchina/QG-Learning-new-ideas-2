...
var ocr_choice = 1;//修改默认OCR为Google
...
// console.show();
fInfo("学习助手" + newest_version + "脚本初始化");
// 初始化宽高
...
function xxqg(userinfo) {
  ...
  // true == bendi && ("old" == jifen_flag && "已完成" != jifen_list.findOne(textEndsWith("本地频道")).parent().child(3).text() || "old" != jifen_flag && "已完成" != jifen_list.findOne(textEndsWith("本地频道")).parent().child(4).text()) && (toastLog("本地开始"),do_bendi());
  //订阅部分
  // d = 1;
  ...
  var token2 = "a0e9394398a34c15831606871e5f96f9";
  if (pushplus || token) {
    fInfo("推送前等待积分刷新5秒");
    sleep(5E3);
