...
if (storage.get(engine_version, true)) {
  ...
  let gengxin_rows = "脚本有风险，仅供学习交流;更新内容：;1.更换稳定代理网站;2.增加优化防检测模块文章、视听、本地随机任务;BUG可提交给机器人;3.脚本测试环境：强国V2.48.0-2.53;4.更新适配雷电模拟器9（点击取消不再提示）".split(";");
...
function do_exec(type) {
  ...
  if (textStartsWith("单选题").exists()) {
    ...
    //let que_txt = className("android.view.View").depth(11).findOnce(1).parent().parent().child(1).text();
    log(que_txt);
